...
class Follower:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber('image_raw', Image, self.image_callback)   #Image型で画像トピックを購読し，コールバック関数を呼ぶ
        self.cmd_vel_pub = rospy.Publisher('cmd_vel', Twist, queue_size = 1)
        self.image_pub = rospy.Publisher("image_lane",Image)
        self.image_mask1 = rospy.Publisher("image_mask1",Image)
        self.image_mask2 = rospy.Publisher("image_mask2",Image)
    ...
    def image_callback(self, msg):
        ...
        lower_yellow = np.array([30, 30, 30])       #黄色の閾値（下限）
        upper_yellow = np.array([255, 255, 250])    #黄色の閾値（上限）
        #lower_yellow = np.array([40, 62, 10])       #黄色の閾値（下限）
        #lower_yellow = np.array([0, 0, 10])       #黄色の閾値（下限）
        #upper_yellow = np.array([70, 92, 255])    #黄色の閾値（上限）
        ...
        try:
            ...
            #self.image_mask1.publish(self.bridge.cv2_to_imgmsg(display_mask, "bgr8"))
            #self.image_mask2.publish(self.bridge.cv2_to_imgmsg(display_masked, "bgr8"))
