...
class PitWheels:
	...
	self.tracked=False
        ...
        self.tracked_y=-50
	self.tracked_ang=0
        self.radius_follow=0.25
        self.yolo_status=0
        self.yolo_dist =0
        self.yolo_ang=0
    
    def pubobs(self):
        if(time.time()-self.last_time>0.09):
        #if(self.track==0):
    ...
    def obstacles_callback(self, data):
        ...
        for x in data.circles:
            if x.center.x>-0.5 and x.center.x<self.front_detection and x.center.y<self.side_detection and x.center.y>-self.side_detection: 
                ...
                if self.tracked_x != -50 and self.tracked_y != -50 and x.center.x<=(self.tracked_x+self.radius_follow) and x.center.x>=(self.tracked_x-self.radius_follow) and x.center.y <=(self.tracked_y+self.radius_follow) and x.center.y >=(self.tracked_y-self.radius_follow):#then we have tracked object
                    e_x=np.abs(self.tracked_x-x.center.x)
                    e_y=np.abs(self.tracked_y-x.center.y)
                    dist=np.sqrt(e_x*e_x+e_y*e_y)
                    #rospy.logerr('cx,cy {},{} tr {},{} di {}'.format(x.center.x,x.center.y,self.tracked_x,self.tracked_y,dist))
        ...
        if len(lst)!=0:
            for n in (lst):
                ...
                #rospy.logerr('t_ang {} an {}'.format(self.tracked_ang,ang_obj))
                #rospy.logerr('t_ang {} an {}'.format(self.tracked_ang,ang_obj))
                
                if dist<val_aux and np.abs(err_ang)<ang_aux: #and err_ang<7 and err_ang>-7 :
                    val_aux=dist
                    ang_aux=err_ang
            ...
            if len(lst2)!=0:
                for n in (lst2):
                    ...
                    if dist<dst_aux: #and err_ang<7 and err_ang>-7 :
                        ...
                        e_y=np.abs(self.tracked_y-n.center.y)
                        dist=np.sqrt(e_x*e_x+e_y*e_y)
                        #rospy.logerr('sent {},{} tr{},{} di{}'.format(n.center.x,n.center.y,self.tracked_x,self.tracked_y,dist))
                        #rospy.logerr('sent {},{} tr{},{} di{}'.format(n.center.x,n.center.y,self.tracked_x,self.tracked_y,dist))
                        ang_obj= 90-(np.arctan2(n.center.x, n.center.y) * 180 / np.pi)
                        #rospy.logerr('diff {},{},{}'.format(self.yolo_ang,ang_obj,self.yolo_ang-ang_obj))
                        
                #rospy.logerr('closest {} , {}'.format(closest.x,closest.y))
                #rospy.logerr('dist {}'.format(dist))
