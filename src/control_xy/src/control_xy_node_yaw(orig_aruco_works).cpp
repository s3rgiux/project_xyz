...
void setPointsCallback(const geometry_msgs::Twist& twist)
    ...
    void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
	{
	    ...
	    float  break_distance=0.7;

		// if (scan->ranges[0] <= break_distance||scan->ranges[1] <= break_distance||scan->ranges[2] <= break_distance||scan->ranges[3] <= break_distance||scan->ranges[4] <= break_distance||scan->ranges[5] <= break_distance||scan->ranges[359] <= break_distance||scan->ranges[358] <= break_distance||scan->ranges[357] <= break_distance||scan->ranges[356] <= break_distance||scan->ranges[355] <= break_distance){

//if (scan->ranges[0] <= break_distance||scan->ranges[1] <= break_distance||scan->ranges[2] <= break_distance||scan->ranges[3] <= break_distance ||scan->ranges[359] <= break_distance||scan->ranges[358] <= break_distance||scan->ranges[357] <= break_distance){
//if (scan->ranges[180] <= break_distance||scan->ranges[181] <= break_distance||scan->ranges[182] <= break_distance||scan->ranges[183] <= break_distance ||scan->ranges[179] <= break_distance||scan->ranges[178] <= break_distance||scan->ranges[177] <= break_distance){

if (scan->ranges[0] <= break_distance){
			 //obstacle_front=true;
			ROS_INFO("Obstacle");
		 }else{
			 obstacle_front=false;
			...
			/*for(int i = 0; i < (6/2)+1; i++){
				if (scan->ranges[i] <= 0.45){
					//ROS_INFO("Obstacle");
					obstacle_front1=true;
					//return;
			    }else{obstacle_front1=false;}
			}
			for(int k = (360-(6/2)); k < count; k++){
				if (scan->ranges[k] <= 0.45){
					//pubEmergencyStop_.publish(speed);
					//ROS_INFO("Obstacle");
					obstacle_front2=true;
					return;
			    }else{obstacle_front2=false;}
			}
			if(obstacle_front1==true||obstacle_front2==true){
				obstacle_front=true;
				ROS_INFO("Obstacle");
			}else{obstacle_front=false;}
	*/	
	}
