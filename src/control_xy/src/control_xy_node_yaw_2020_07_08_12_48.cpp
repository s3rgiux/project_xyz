		   ...
		   if(msg.data!=-500){
			...
			if (msg.data>-5 && msg.data<5&& distanciaPeople2<90){
			   ...
			   if(distanciaPeople2>100 && distanciaPeople2<=150){
				...
				//vel_steer.linear.x=frontal_gain_follow*distanciaPeople2;
				ctrl_front_follow=frontal_gain_follow*distanciaPeople2-ctrl_front_follow/2;
				if (ctrl_front_follow>(max_speed_follow/2)){
                          		ctrl_front_follow= max_speed_follow/2;
				}
...
private:
	...
	ros::Time time_obst,time_cruce,time_obst_last,time_cruce_last;
        float errpx,errpy,spVel,nkp,nkd,nki,errVelYaw,nvkp,nvkd,spPos,distAct,errPVel,spPVel,ctrl_vel,frontal_gain_follow,frontal_gain_karugamo,ctrl_front_follow,ctrl_front_karugamo;
