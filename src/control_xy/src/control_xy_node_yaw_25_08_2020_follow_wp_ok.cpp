...
void angPeopCallback3(const geometry_msgs::Vector3& msg){
		...
		if(mode_follow && danger!=true){ //&& tracking_people){//1
		   ...
		   //if( cx!=0.01 && cy !=0.01 && cx<=(tracked_cx+radius_follow) && cx>=(tracked_cx-radius_follow) && cy<=(tracked_cy+radius_follow) && cy >=(tracked_cy-radius_follow)){//2
		    if( cx!=0.01 && cy !=0.01 && ang_peop_lidar<80 && ang_peop_lidar>-80 && tracking_people){//2
			ang_peop_lidar = 90 + atan2(cx, cy) * 180 / 3.1416;
			distanciaPeople2 = sqrt(cx*cx+cy*cy)*100;
                        ROS_INFO("T%f,%f",tracked_cx,tracked_cy);
                        tracked_angle= ang_peop_lidar;
		...
		if(mode_follow && danger!=true){ //&& tracking_people){//1
		   ...
		   //ROS_INFO("P%f,%f",tracked_cx,tracked_cy);
			...
			if(sqrt((npcx-prev_cx)*(npcx-prev_cx)+(npcy-prev_cy)*(npcy-prev_cy))>0.45){
				...
				//aux1= spx_follow[2];
				//aux2= spy_follow[2];
				spx_follow[1]=spx_follow[2];
				spy_follow[1]=spy_follow[2];
				spx_follow[2]=npcx;
				spy_follow[2]=npcy;
				//spx_follow[0]=npcx;
				//spy_follow[0]=npcy;
			...
			////////////
		if(cont_sp_follow>=2 && distanciaPeople2>120 && kf < cont_sp_follow){
		//if(cont_sp_follow>=0 ){
...
private:
	...
	float spx_follow[250000],spy_follow[250000],prev_cx,prev_cy;
