...
public:
	test_head(ros::NodeHandle nh)
	{
        ...
        speed_publisher=nh.advertise<geometry_msgs::Twist>(nh.resolveName("/cmd_vel"), 2);
	        ...
	        //head_subscriber = nh.subscribe("/euler2", 1, &test_head::headCallback,this);
...
void setPointsCallback(const geometry_msgs::Twist& twist)
	...
	void angPeopCallback(const std_msgs::Float32& msg){
		...
		if(mode==3){
			...
			//vel_steer.linear.x=-joy->axes[1]*0.6;
			
			//if (msg.data<75 && msg.data>45){
			//	vel_steer.angular.z=24;
			//}else if (msg.data>100 && msg.data<140){
			//	vel_steer.angular.z=-24;
			//}else{
			//	vel_steer.angular.z=0;
			//}
			
			if (msg.data<-7 && msg.data<-7&& distanciaPeople<50){
                        vel_steer.angular.z=0;
                        }else{	
			vel_steer.angular.z=0.4*msg.data;
			if (vel_steer.angular.z>35){
                          vel_steer.angular.z= 35;
			}else if (vel_steer.angular.z<-35){
                          vel_steer.angular.z= -35;
			}
                        }
			//vel_steer.angular.z=0;
			if(distanciaPeople>50){
				vel_steer.linear.x=90;
			}else{
				vel_steer.linear.x=0;
			}
...
*/
		...
		if(mode==1){
			...
			vel_steer.linear.x=-joy->axes[1]*-100;
			vel_steer.angular.z=joy->axes[0]*-50;
