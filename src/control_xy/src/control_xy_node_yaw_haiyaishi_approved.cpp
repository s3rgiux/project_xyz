	...
	void angPeopCallback(const std_msgs::Float32& msg){
		...
		if(mode_karugamo && danger!=true){
		   if(msg.data!=-500){
			...
			if (msg.data>-5 && msg.data<5&& distanciaPeople<95){
                        	vel_steer.angular.z=0;
                        }else{	
			   ...
			   if(distanciaPeople>95 && distanciaPeople<=150){
				...
				vel_steer.linear.x=4*distanciaPeople;
				if (vel_steer.linear.x>(max_speed_karugamo/2)){
                          		vel_steer.linear.x= max_speed_karugamo/2;
				}
			   }else if(distanciaPeople>150){
				//vel_steer.linear.x=120;
				vel_steer.linear.x=7*distanciaPeople;
				if (vel_steer.linear.x>max_speed_karugamo){
                          		vel_steer.linear.x= max_speed_karugamo;
                                }
			   }else{
				vel_steer.linear.x=0;
			   }
...
void angPeopCallback2(const std_msgs::Float32& msg){
		...
		if(mode_follow && danger!=true){
		   if(msg.data!=-500){
			geometry_msgs::Twist vel_steer;		
			if (msg.data>-5 && msg.data<5&& distanciaPeople2<90){
                        	vel_steer.angular.z=0;
                        }else{	
				
				if(distanciaPeople2<120){
					ctrl_ang= low_vel_gain_follow*msg.data-ctrl_ang/2;
					vel_steer.angular.z=ctrl_ang;//low_vel_gain*msg.data;//*(distanciaPeople/40);
				   	if (vel_steer.angular.z>400){
                        			vel_steer.angular.z= 400;
				  	}else if (vel_steer.angular.z<-400){
				   	...
				   	//vel_steer.angular.z=15*msg.data - vel_steer.angular.z/2;//*(distanciaPeople/40);
					ctrl_ang= high_vel_gain_follow*msg.data-ctrl_ang/2;
					vel_steer.angular.z=ctrl_ang;//vel_steer.angular.z=high_vel_gain*msg.data;
				   	if (vel_steer.angular.z>500){
                        			vel_steer.angular.z= 500;
				  	}else if (vel_steer.angular.z<-500){
			   ...
			   if(distanciaPeople2>100 && distanciaPeople2<=150){
				//vel_steer.linear.x=120;
				vel_steer.linear.x=3*distanciaPeople2;
				if (vel_steer.linear.x>(max_speed_follow/2)){
                          		vel_steer.linear.x= max_speed_follow/2;
				}
			   }else if(distanciaPeople2>150){
				//vel_steer.linear.x=120;
				vel_steer.linear.x=7*distanciaPeople2;
...
private:
	...
	float px,py,norm,norm2,x_ini,y_ini,x_p,y_p,px2,py2,spx,spy,err_x,err_y,err_xx,err_yy,low_vel_gain,high_vel_gain,low_vel_gain_follow,high_vel_gain_follow;
