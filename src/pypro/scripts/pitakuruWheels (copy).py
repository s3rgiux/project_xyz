...
class PitWheels:
    def __init__(self):
        ...
        #self.left_w_dev.set_max_torque(4.0)
        #self.left_w_dev.set_led(1, 0, 200, 0)
        #self.left_w_dev.set_acc(ACC)
        #sleep(1.0)
        # self.left_w_dev.start_serial_reading()
        ...
        #self.right_w_dev.set_max_torque(4.0)
        #self.right_w_dev.set_led(1, 0, 200, 0)
        #self.right_w_dev.set_acc(ACC)

        #sleep(1.0)
        # self.right_w_dev.start_serial_reading()
    ...
    def correct_callback(self, data):
        self.yPosLas =-data.pose.pose.position.x
        self.xPosLas =-data.pose.pose.position.y
    ...
    def run_ctrl_cmd(self):
        ...
        if self.left_velocity<lim and self.left_velocity>-lim:
            ...
            self.left_w_dev.runvel_handler(lim)
        elif  self.left_velocity<-lim:
            self.left_w_dev.runvel_handler(-lim)
        ...
        if self.right_velocity<lim and self.right_velocity>-lim:
            ...
            self.right_w_dev.runvel_handler(lim)
        elif  self.right_velocity<-lim:
            self.right_w_dev.runvel_handler(-lim)
        #self.left_w_dev.runvel_handler(int(self.left_velocity))
        #rospy.sleep(0.02)
